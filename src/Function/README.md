...
The Amazon Event Bridge formerly known as Cloud Watch events triggers the lambda function via a scheduled rule , which then processes the script and the results are stored in the S3 bucket. The bucket key is formed as such <bucket-name>/<year>/<month>/<date>/hour
